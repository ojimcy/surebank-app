...
function PaymentError() {
  ...
  if (transactionReference) {
    if (
      transactionReference.startsWith('ds_') ||
      transactionReference.startsWith('sb_')
    ) {
      paymentType = 'contribution';
    } else if (transactionReference.startsWith('ibs_')) {
      paymentType = 'ibs';
    }
  } else {
    paymentType = searchParams.get('type') || 'other';
  }

  const handleTryAgain = () => {
    if (paymentType === 'contribution') {
      navigate('/payments/deposit');
    } else if (paymentType === 'ibs') {
